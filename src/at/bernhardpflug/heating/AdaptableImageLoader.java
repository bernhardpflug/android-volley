package at.bernhardpflug.heating;
...
/**
 * Hard copy of ImageLoader to allow choosing socket timeout Only change is usage of AdaptableImageRequest in method get(String, ImageListener)
 */
public class AdaptableImageLoader {
	 ...
	 */
	public AdaptableImageLoader(RequestQueue queue, ImageCache imageCache, int socketTimeout) {
		mRequestQueue = queue;
		mCache = imageCache;
	 ...
	 * 
	 * 2. After a network response returns, only one of the following cases will happen: - onResponse(response, false) will be called if the image was loaded. or - onErrorResponse will be called if there was an error loading the image.
	 */
	 ...
	 * 
	 * The ImageContainer will contain either the specified default bitmap or the loaded bitmap. If the default was returned, the {@link ImageLoader} will be invoked when the request is fulfilled.
	 * 
	 * @param requestUrl
	 ...
	 * @param maxHeight
	 *            The maximum height of the returned image.
	 * @return A container object that contains all of the properties of the request, as well as the currently available image (default if remote is not loaded).
	 */
	public ImageContainer get(String requestUrl, ImageListener imageListener, int maxWidth, int maxHeight) {
		...
		// TODO added
		newRequest.setRetryPolicy(new DefaultRetryPolicy(socketTimeout, 2, 2f));
